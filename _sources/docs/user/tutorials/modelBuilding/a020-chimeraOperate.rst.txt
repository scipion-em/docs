...
this protocol to perform rigid fitting in *Scipion* by using *ChimeraX* (Appendix :ref:`Rigid fit <app:chimeraRigidFit>`).
...
-  Protocol form parameters (:numref:`model_building_app_protocol_chimera_2` (B)):
   ...
   -  *Input* section
      ...
      -  *Input Volume*: Optional parameter to be completed with the
         electron density map previously downloaded or generated in *Scipion*.

      -  *Input additional Volumes*: Optional parameter to include other
         electron density maps previously downloaded or generated in *Scipion*.

      -  *Atomic structure*: Atomic structure previously downloaded or
         generated in *Scipion*.
   ...
   -  *Help* section
      ...
      required. Remark that using *scipionwrite* command, *ChimeraX* session will
      ...
      *scipionss* command. *scipioncombine* command allows to merge in
      only one atomic structure two or more structures. *ChimeraX* sessions can be restored by
      using **chimerax-restore session** protocol.
