...
-  Protocol form parameters (:numref:`model_building_app_protocol_extractUnitCell_1` (B)):
   ...
   -  *Input Volume*: Volume already downloaded in *Scipion* from which the
   ...
   -  *Symmetry*: In this protocol, symmetry refers only to rotational symmetry, also known in biology as radial symmetry. This symmetry is the property of volumes to preserve their shape after a partial turn around a symmetry axis.
    | Types of rotational symmetry included in this protocol are shown
      ...
      -  Icosahedral symmetries *I1 (I222), I2 (I222r), I3 (In25), I4
         ...
         initial orientation. Check in `CHIMERAX <https://www.cgl.ucsf.edu/chimerax/docs/user/commands/shape.html>`_ each icosahedral symmetry by writing in the command line:
         *shape icosahedron radius 50 orientation* (*222*: default,
         ...
         follow Y and Z axes, respectively, *n25r*: idem rotated 180º
         around X axis.
...
-  Visualization of protocol results:
   ...
   window will be opened (:numref:`model_building_app_protocol_volume_2`). This window allows you to select between
   *chimerax* (*ChimeraX* graphics window) and *slices*
   (*ShowJ*, the default *Scipion* viewer), to visualize the volume.
   ...
   -  *chimerax*: *ChimeraX* graphics window
      ...
      origin of coordinates in *ChimeraX*. To show the relative position of the
      volume, the three coordinate axes are represented; X axis (red), Y
      axis (yellow), and Z axis (blue) (:numref:`model_building_app_protocol_volume_3`). Coordinate axes, initial
      volume, and extracted map asymmetric unit are model numbers *#1*,
      *#2* and *#3*, respectively, in *ChimeraX Models* panel.
      Volume coordinates and pixel size can be checked in
      *ChimeraX* main menu *Tools -> Volume Data -> Map
