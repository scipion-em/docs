...
Protocol designed to assess in *Scipion* the geometry of refined atomic structures
without considering electron density maps by using :math:`MolProbity` :cite:p:`davis2004`. Integrated in cryo-EM validation tools of `PHENIX software suite <https://www.phenix-online.org/>`_,
...
input in *Phenix MolProbity* protocol is possible for *PHENIX* v. 1.13 and *Real
...
-  | *Scipion* menu: *Model building -> Validation* (:numref:`model_building_app_protocol_molprobity_1` (A))

   .. figure:: Images_appendix/Fig143.svg
      :alt: Protocol **phenix-molprobity**. A: Protocol location in *Scipion* menu. B: Protocol form.
      :name: model_building_app_protocol_molprobity_1
      :align: center
      :width: 90.0%

      Protocol **phenix-molprobity**. A: Protocol location in *Scipion* menu. B: Protocol form.

-  Protocol form parameters (:numref:`model_building_app_protocol_molprobity_1` (B)):
...
-  Visualization of protocol results:
   ...
   .. figure:: Images_appendix/Fig144.svg
      :alt: Protocol **phenix-molprobity**. Taps to visualize :math:`MolProbity` and Real space correlation results. *Real-space correlation* tap only appears with *PHENIX* v. 1.13.
      :name: model_building_app_protocol_molprobity_2
      :align: center
      :width: 50.0%

      Protocol **phenix-molprobity**. Taps to visualize :math:`MolProbity` and Real space correlation results. *Real-space correlation* tap only appears with *PHENIX* v. 1.13.

   Four taps are shown in the upper part of the results window:

   -  | *Volume and models*: *ChimeraX* graphics window will be opened by default. Volume and atomic structure, if it is present, are referred to the origin of coordinates in *ChimeraX*. To show the relative position of atomic structure and electron density volume, the three coordinate axes are represented; X axis (red), Y axis (yellow), and Z axis (blue) (:numref:`model_building_app_protocol_volume_3`).

   -  *MolProbity results* (:numref:`model_building_app_protocol_molprobity_3`):

      .. figure:: Images_appendix/Fig145.svg
         :alt: Protocol **phenix-molprobity**. :math:`MolProbity` results.
         :name: model_building_app_protocol_molprobity_3
         :align: center
         :width: 50.0%

         Protocol **phenix-molprobity**. :math:`MolProbity` results.

      -  *Summary MolProbity*:

         -  | *MolProbity Basic Statistics*: Statistics computed by :math:`Phenix` package to assess protein geometry using the same distributions as the MolProbity server:
            ...
            -   | *Rotamer outliers*: Percentage of residues assessed that adopt an unusual conformation of :math:`\chi` dihedral angles. Rotamer outliers, commonly used to characterize the conformation of protein sidechains, are detailed in Chi1-Chi2 plot, shown below.
            ...
            -   | *Overall score*: :math:`MolProbity` overall score represents the experimental resolution expected for the structure model. This value should be lower than the actual resolution. The lower the value, the better quality of the structure model.

         -  | *Open in Coot*: Interactive visualization and structure modification tool for Ramachandran, Rotamer and C\ :math:`\beta` outliers, as well as severe clashes. *Coot* graphics window will be centered on the specific atom or residue outlier when it is clicked. Improvements of the atomic structure are allowed in and any modification can be saved in  *Scipion* as usual (look at *Help section: Saving an atomic structure after an interactive working session with Coot* (Appendix :ref:`Coot refinement <app:ccp4CootRefinement>`). The interactive protocol box will appear hanging out of *MolProbity*.
      ...
      -  | *Protein*: Box validating protein geometry:
         ...
         -  | *Ramachandran outliers*: List of Ramachandran residue outliers with their respective :math:`\phi` (C-N-CA-C) and :math:`\psi` (N-CA-C-N) dihedral angle values.

         -  | *Rotamer outliers*: List of Rotamer residue outliers with their respective :math:`\chi` dihedral angles.

         -  | *C-beta outliers*: List of C\ :math:`\beta` residue outliers with their respective angles (angular position of *C-beta* atom in radial space).
         ...
         -  | *Cis and Twisted peptides*: Residues showing :math:`cis` or :math:`twisted` conformations that could be modeling errors.
   ...
   -  | *Real-space correlation*: (This tap will only appear with *PHENIX* v. 1.13 in case you include a electron density volume as input of the protocol) (:numref:`model_building_app_protocol_molprobity_4`):

      .. figure:: Images_appendix/Fig146.svg
         :alt: Protocol **phenix-molprobity**. Real-space correlation results.
         :name: model_building_app_protocol_molprobity_4
         :align: center
         :width: 50.0%

         Protocol **phenix-molprobity**. Real-space correlation results.

      -  | *Real-space correlation to electron density*:

         -  | *View Multi-criterion plot*: Plot showing cross-correlation and B-factor values for each residue of the macromolecule over 100-residue regions. Additional validation information, such as Ramachandran, Rotamer or C\ :math:`\beta` outliers, is also detailed, as well as severe clashes.

         -  | *Correlations Coefficients*: Three Real-space correlation coefficients are computed :cite:p:`afonine2018b`:
   ...
   -  | *Atomic properties* (:numref:`model_building_app_protocol_molprobity_5`): Atom numerical properties:

      .. figure:: Images_appendix/Fig147.svg
         :alt: Protocol **phenix-molprobity**. Atomic properties results.
         :name: model_building_app_protocol_molprobity_5
         :align: center
         :width: 50.0%

         Protocol **phenix-molprobity**. Atomic properties results.
...
-  Summary content:
   ...
   | Main statistics included in the above :math:`MolProbity` *Model
     Final Statistics* table (an example can be seen in :numref:`model_building_molprobity_protocol` (7)).
