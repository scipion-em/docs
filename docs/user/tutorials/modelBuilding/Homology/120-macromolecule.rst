...
-  | Validation protocols to select the best *model* of the whole
   ...
   | *EMRinger* and *Validation CryoEM (MolProbity)* statistics have to be
     ...
     obtained by using *ChimeraX operate* protocol (see results Table 9 in Appendix :ref:`Solutions <app:solutions>`; :ref:`Question12 <question12>`). Because of
...
-  | Protocol **chimerax-map subtraction** to assess volume density occupancy:
   ...
   | We perform this analysis in order to identify parts of the density
     ...
     aproximation, we will run a protocol based on *ChimeraX* (see Appendix :ref:`CHIMERAX Map subtraction <app:chimeraMapSubtraction>` with use cases) to subtract the modeled part of the map from the whole
     map.
