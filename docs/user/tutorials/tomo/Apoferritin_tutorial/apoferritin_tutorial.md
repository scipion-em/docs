...
> Note: This auto.refine is not neccesary, alternatively, the inital model can be upsampled to continue a refinement att bin 4. However, the model is refined in this tutorial to illustrate the change of binning.
...
![extractbin4](Figures_ApoferritinTutorial/extractbin4.png)
...
![extractbin4](Figures_ApoferritinTutorial/extractbin2.png)
...
![relionReconstructParticlebin2](Figures_ApoferritinTutorial/relionReconstructionBin2.png)
...
The reconstructed protein can be visualized with Scipion (to see the slices) or with Chimera (to see the 3D map). 

![reconstructParticlebin2Result](Figures_ApoferritinTutorial/reconstructionRelionTomobin2.png)
...
- **Pseudo-subtomograms**: The 2D extracted ones at bin 2
- **Reference Volume**: The reconstructed volume at bin 2 from the previous step
- **Reference Mask**: Leave empty, we lack of resolution for it
- **Is initial 3D map on absolute greyscale?1**: Yes
- **Resize references if needed?**: Yes
- **Initial low-pass filter (A)**: 7A
- **Symmetry group**: O. In this case the protein has O symmetry
- **Do CTF-correction?**: Yes
- **Ignore CTF until first peak?**: No
- **Circular Mask diameter**: 150A.
